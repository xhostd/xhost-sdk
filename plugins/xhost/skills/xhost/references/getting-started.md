...
- **Local git workflow (Claude Code only):** if the user wants to push from a local checkout, call `mcp__xhost__get_git_credentials`, configure `git remote add xhost https://<username>:<token>@git.xhostd.com/<username>/<app>.git` (token in the password field — any username works), `git push`, then `mcp__xhost__deploy` with `ref="master"`. The token expires after 30 days and cannot do anything except git push/pull.
