...
3. **`mcp__xhost__deploy`** — args: `app_name`, `channel` (e.g. `"prod"`), and either `ref` (a branch name, e.g. `"master"`; xhostd resolves it to that branch's current head — this is the form to use after a push) or `sha` (an exact commit — what `commit_files` returned). Returns `{deploy_id, channel_id, status: "queued"}`.
